...
def register():
    ...
    if request.method == 'POST':
        ...
        if existing_user:
            return render_template('register.html', user=user, error ='USER ALREADY EXIST! PLESE CHOOSE ANOTHER NAME')
...
def question(question_id):
    user = get_current_user()
    db=get_db()
    question_cur= db.execute('select questions.answer_text, questions.question_text, askers.name as asker_name, experts.name as expert_name from questions join users as askers on askers.id = questions.asked_by_id join users as experts on experts.id = questions.expert_id where questions.id=?',[question_id])
